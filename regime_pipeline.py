...
def main() -> None:
    ...
    rule_series = rule_based_history(spy)
    rule_series = rule_series[rule_series.index >= cutoff]
    rule_hist = {ts.strftime("%Y-%m-%d"): lbl for ts, lbl in rule_series.items()}
    ...
    history = {
        "as_of": as_of.strftime("%Y-%m-%d"),
        "generated_at": generated_at,
        "hmm": hmm_window.to_dict(orient="records"),
        "rule_based": [
    ...
    print(f"Wrote {out_dir/'latest.json'} and {out_dir/'history.json'}")
